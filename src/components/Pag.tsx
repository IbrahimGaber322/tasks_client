...
export default function Pag() {
  ...
  const {numberOfPages, currentPage } = useSelector((state:any)=>state?.tasks);
  ...
  return ( currentPage&&
       <Box borderRadius={3} p={1} sx={{backgroundColor:(theme)=>theme.palette.primary.dark , mt:"auto", width:"fit-content", mx:"auto", mb:2}}>
      <Pagination  page={Number(page)} renderItem={(item) => <PaginationItem  to={!(searchQuery||searchTags)?`/?page=${item.page}`:`/tasks/search?searchQuery=${searchQuery}&searchTags=${searchTags}&page=${item.page}`} component={Link} {...item} />} size='large' count={numberOfPages} defaultPage={1} color={'primary'} />
      </Box>
  )
}